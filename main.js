...
function calcRange(stats) {
    ...
    let mastery = masteryElement ? parseFloat(masteryElement.innerText) || 0.1 : 0.1;
...
function addChangeListenerAndUpdateDisplay(selectElement, skillList, selectedKeyIndex, displayId) {
    if (!document.getElementById(displayId).textContent.trim()) {
        ...
        document.getElementById(displayId).textContent = initialValue;
    }
...
function inputSelectToTable(list, selectId) {
    ...
    // let selectHTML = "<select id='" + selectId + "' onchange='loadUserStats()'>";
...
for (i = 0; i < monsterlist.length; i++) {
    ...
    option.value = i;
    option.text = "LV." + monsterlist[i].level + " " + monsterlist[i].text;
...
function displayDamage(userLvl, int, luk, totalmagic, totalATT, maxRange, minRange, maxRange2, minRange2) {
    ...
    const BASIC_DAMAGE = BASIC_DAMAGE_Element ? parseFloat(BASIC_DAMAGE_Element.innerText) : 0;
    const MAGIC_MASTERY_Element = document.getElementById("value4Display");
    const MAGIC_MASTERY = MAGIC_MASTERY_Element ? parseFloat(MAGIC_MASTERY_Element.innerText) : 0.1;
    const CRIT_DAMAGE_Element = document.getElementById("value5Display");
    const CRIT_DAMAGE = CRIT_DAMAGE_Element ? parseFloat(CRIT_DAMAGE_Element.innerText) || 1 : 1;
    const CRIT_PROB_Element = document.getElementById("value2Display");
    const CRIT_PROB = CRIT_PROB_Element ? parseFloat(CRIT_PROB_Element.innerText) : 4;
...
function displayProb(maxDamage, minDamage, maxCritDamage, minCritDamage, CRIT_PROB, maxDamage2, minDamage2,
        ...
        case "warrior":
            switch (jobs) {
                ...
                case "two_handed_sword":
                    ...
                    + 2*CRITICAL_THROW_PROB*(1-CRIT_PROB) * calcDoubleProbability(MEAN_of_MIN, MEAN_of_MAX, monsterHp)
                ...
                case "spear":
                    ...
                    for (let i=1; i<10; i++) {
                        HitProb7[i] = 0.6*calcMultipleProbability(2*i+2, minDamage, maxDamage2, minCritDamage, maxCritDamage2, monsterHp, CRIT_PROB)
                        + 0.4*calcMultipleProbability(2*i+2, minDamage2, maxDamage, minCritDamage2, maxCritDamage, monsterHp, CRIT_PROB)
                    }
                    break;
                case "polearm":
                case "one_handed_axe":
                case "two_handed_axe":
                    ...
                    for (let i=1; i<10; i++) {
                        HitProb7[i] = 0.4*calcMultipleProbability(2*i+2, minDamage, maxDamage2, minCritDamage, maxCritDamage2, monsterHp, CRIT_PROB)
                        + 0.6*calcMultipleProbability(2*i+2, minDamage2, maxDamage, minCritDamage2, maxCritDamage, monsterHp, CRIT_PROB)
                    }
                    break;
            }
            break;
    ...
    console.log(minCritDamage, HitProb7, CRIT_PROB)
    for (let i=0; i<10; i++) {
        if (HitProb7[i] < 0) {
            HitProb7[i] = 0;
    ...
    for (let i=0; i<10; i++) {
        document.getElementById("HitProb"+i).innerText = i+1 + "방컷 확률 : " + (100*HitProb[i]).toFixed(2) + "%";
